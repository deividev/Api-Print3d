...
const OnDBReady = (err) => {
  ...
  app.post("/api/upload", filesUpload, async (req, res) => {
    ...
    const imgName = `${express.static(path.join(__dirname, '/publicc/uploads/images/'))}${req.files.image[0].filename}`;
    const modelName = `${express.static(path.join(__dirname, '/publicc/uploads/images/'))}${req.files.model[0].filename}`;
